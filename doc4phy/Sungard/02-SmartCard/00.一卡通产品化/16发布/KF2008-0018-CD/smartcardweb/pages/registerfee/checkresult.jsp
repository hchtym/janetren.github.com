...
<html>
	<head>  
		...
		<link href="/smartcardweb/pages/css/css.css" rel="stylesheet"
			...
			String feeinfo = request.getAttribute("feeinfo") == null ? "":request.getAttribute("feeinfo").toString();
            String sdate =   request.getAttribute("sdate") == null ? "": request.getAttribute("sdate").toString();//需要确认是否是 yymmdd
		...
		function writeCard(){
				...
				var ret=checkresultForm.all.RFScanner.UpdateCardInfo(<%=cardNo%>, <%=feeinfo%>, "<%=sdate%>", "<%=paramKey%>");
				if(ret == 0){ 
				    document.checkresultForm.errmsg.value="更新卡信息成功!";
				    checkresultForm.submit();
				}else{ //失败
				     document.checkresultForm.errmsg.value="更新卡信息失败,稍候再试!";
					 checkresultForm.submit();
				}
			}
		</script>
	...
	<body scroll="no">
		<form action="/smartcardweb/pages/registerfee/returninfo.jsp" method="post"
				...
				<div id="Layer1">
					<div id="content">
						...
						<div id="content_middle">
							<div align="center">
								<img src="/smartcardweb/pages/images/zz2.gif" width="314"
									height="82" />
							</div>
							...
							<div id="content_input">
								<table width="80%" border="0" align="center" cellpadding="0"
									...
									<td height="35" class="样式13">
                                           学/工号
											<input type="hidden" name="stuempno"
												value="<%=request.getAttribute("stuempno") == null ? ""
					...
					<input type="hidden" name="custno"
												value="<%=request.getAttribute("custno") == null ? ""
					: request.getAttribute("custno")%>" />
					                </td>
									  <td>
									
											<%=request.getAttribute("stuempno") == null ? "" : request
					.getAttribute("stuempno")%>
										</td>
										</tr>
									<tr>
										<td height="35" class="样式13">
										...
										<td>
											<%=request.getAttribute("name") == null ? "" : request
					.getAttribute("name")%>
