...
<html>
	...
	<script language="javascript">
		...
		var ret ;
		var rret = writeCardForm.all.RFScanner.turnoverwritecard("","<%=cardno%>","");//写卡
		if(rret == 0){
			 var rrret = writeCardForm.all.RFScanner.turnoverchecked(serverbase+"turnoverchecked.do","","");//系统入账  修改缴费记录
			 if(rrret == 0){//缴费成功
			 	writeCardForm.all.cardId.value=writeCardForm.all.RFScanner.GetRequestRetInfo("cardno");
			 	writeCardForm.submit();
			 }else{
			 	window.location='/smartcardweb/pages/bmjf/dealerror.jsp';
			 }
		}else{
			window.location='/smartcardweb/pages/bmjf/cardwriteerror.jsp';
		}	
	}
