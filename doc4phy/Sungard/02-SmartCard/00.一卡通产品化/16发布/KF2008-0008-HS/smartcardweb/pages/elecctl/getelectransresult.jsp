...
<html>
	<head>
		...
		function writeCard(){
			...
			var parms = "money="+t_money+"&serialNo="+serial_no+"&room="+myroom;
			writeCardForm.all.RFScanner.PortNo=device_port;
			var ret = writeCardForm.all.RFScanner.ElectTransWriteCard(url,card_id,out_card_amount,parms);
			if(ret==0){
				msg.innerHTML="电控转账成功!<br/><br/>转账金额 "+t_money+" 元.<br/><br/>卡余额 "+parseFloat(out_card_amount)/100+" 元";
			}else if(ret==2){
				msg.innerHTML="电控转账失败!<br/><br/>失败原因：网络超时！";
			}else if(ret==-2){
				msg.innerHTML="电控转账失败!<br/><br/>失败原因：写卡失败！";
			}else if(ret==-3){
				msg.innerHTML="电控转账失败!<br/><br/>失败原因：请求无应答！";
			}else{
				msg.innerHTML="电控转账失败!<br/><br/>失败原因："+writeCardForm.all.RFScanner.GetRequestRetInfo("errmsg");
			}
		}
