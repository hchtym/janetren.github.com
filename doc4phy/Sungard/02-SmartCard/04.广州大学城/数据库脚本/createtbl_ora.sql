...
alter table YKT_WEB.WIKIPAGE
   add constraint SQL051103095646100 primary key (NODEID, TITLE, VERSION)
    ...
    (
     ...
     logging;
create table YKT_CUR.T_CARDUSEINFO
(
...
comment on column T_CARDUSEINFO.SCHOOL_CODE
  is '校区代码';
comment on column T_CARDUSEINFO.START_NO
  is '客户起始号';
comment on column T_CARDUSEINFO.END_NO
  is '客户结束号';
comment on column T_CARDUSEINFO.CURRENT_NO
  is '客户当前号';
comment on column T_CARDUSEINFO.UNUSE_CNT
  is '最后保存时间';
-- Create/Recreate primary, unique and foreign key constraints 
alter table YKT_CUR.T_CARDUSEINFO
